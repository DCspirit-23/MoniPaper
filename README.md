...
“更多设置”包含暖色、压暗、显示器选择、休息提醒、开机启动、快捷键和关闭窗口的行为。返回主面板时保留当前设置。预览始终显示所选效果，是否应用到桌面以顶部状态为准。
...
<img src="docs/images/tray-menu.png" width="232" alt="托盘菜单：绿色选中状态、圆角悬停背景和宽松间距">
...
| 开机启动 | 在“更多设置”打开后，登录 Windows 时自动驻留托盘并恢复已保存效果 |
| 重新打开面板 | 托盘入口或再次打开程序 |
| 完全退出 | 面板或托盘中的退出入口，同时移除覆盖层 |

休息提醒默认关闭，可选择 20、30、45 或 60 分钟。系统的通知设置可能影响提醒显示。

开机启动默认关闭；开启后在登录 Windows 时生效。如果已在 Windows“启动应用”中禁用 MoniPaper，需要在系统设置中重新允许。软件不会覆盖系统的禁用决定。
...
为保留旧版设置，配置继续保存在当前用户的 `%LOCALAPPDATA%\PaperCare\settings.json`。普通设置更改后自动保存，快捷键在应用成功后保存。开机启动开关写入当前用户的 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\MoniPaper`，启用时使用带引号的当前 EXE 绝对路径并附加 `--startup`；自动启动只恢复效果并驻留托盘，手动打开仍显示面板。移动程序后请关闭再重新开启此开关以登记新路径。程序不会修改显示器硬件亮度或系统色彩配置，也不会自动改写 Windows 的 StartupApproved 状态。
